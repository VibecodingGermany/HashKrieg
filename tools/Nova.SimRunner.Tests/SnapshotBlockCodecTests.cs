...
{
    /// <summary>
    /// Block-level codec: <see cref="SnapshotBlockWriter"/>/
    /// <see cref="SnapshotBlockReader"/> encode canonical little-endian
    /// (Serialization.md section 1) field-identical to
    /// <see cref="SimHashWriter"/>, and the strict reader rejects truncation
    /// on every field boundary without exceptions.
    /// </summary>
    [TestFixture]
    public sealed class SnapshotBlockCodecTests
    {
        [Test]
        ...
        public void Reader_TruncatedAtEveryPrefix_NeverThrows()
        {
            ...
            for (int prefix = 0; prefix <= content.Length; prefix++)
            {
                ...
                // Drain the reader; any prefix must be safe to consume.
