...
{
    /// <summary>
    /// SimulationCore.md section 7, point 1: serialize → deserialize →
    /// serialize yields byte-identical bytes, across block combinations,
    /// empty blocks and arbitrary insertion order. Zero blocks are not a
    /// canonical artifact: the writer throws (structural error) and the
    /// reader rejects with <see cref="SnapshotReadError.EmptyBlockTable"/> —
    /// the documented behavior for that case.
    /// </summary>
    [TestFixture]
    public sealed class SnapshotRoundtripTests
    {
        ...
        {
            byte[] first = SnapshotTestUtil.CreateSampleWriter().ToArray();
            Assert.That(
        ...
        public void Roundtrip_EmptyBlock_IsPreserved()
        {
            ...
            Assert.That(
                parsed.TryGetBlock(SnapshotTestUtil.BlockIdC, out byte[] content), Is.True);
        ...
        {
            byte[] ascending = SnapshotTestUtil.CreateSampleWriter().ToArray();
            ...
            reversed.AddBlock(SnapshotTestUtil.BlockIdC, ReadOnlySpan<byte>.Empty);
            reversed.AddBlock(SnapshotTestUtil.BlockIdB, b);
            var a = new SnapshotBlockWriter();
            a.WriteFieldTag(1);
            a.WriteTick(new Tick(42));
            a.WriteSimFixed(SimFixed.FromInt(-3));
            reversed.AddBlock(SnapshotTestUtil.BlockIdA, a);
        ...
        {
            byte[] first = SnapshotTestUtil.CreateSampleWriter().ToArray();
            byte[] second = SnapshotTestUtil.CreateSampleWriter().ToArray();
        ...
        {
            byte[] file = SnapshotTestUtil.CreateSampleWriter().ToArray();
            SnapshotTestUtil.PatchUInt16(file, 10, 0); // BlockCount field
            Assert.That(
