...
- HART (exit 1): tote interne Links, ungültiges UTF-8, nicht-striktes
  Quality-JSON.
- WEICH (nur Hinweis): fehlende Status-Kopfzeile in docs/.

Seit D-076 (Governance-Tier 1) erzwingt dieser Check den Dokument-Pflichtaufbau
nicht mehr: keine Pflichtabschnitte, kein Versionsbump, keine
Änderungsverlauf-Tabelle. Git ist der Änderungsverlauf. Die Evidence- und
Receipt-Prüfungen des schlafenden Gate-Apparats laufen im integrity-Job von
...
def main() -> int:
    ...
    if missing_header:
        print(
            f"::notice:: {len(missing_header)} docs/-Datei(en) ohne Status-Kopfzeile "
    ...
    if quality_errors:
        print(f"\n::error:: {len(quality_errors)} Quality-JSON-Fehler:")
        for message in quality_errors:
            print(f"  {message}")

    if decode_errors or dead or quality_errors:
        return 1

    print(
        f"OK: {len(paths)} Markdown-Dateien und {len(QUALITY_JSON)} Quality-JSONs "
        "geprüft."
    )
    return 0
